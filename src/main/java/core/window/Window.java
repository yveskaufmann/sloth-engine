...
public class Window implements Cleanable {
	...
	public Window enable() {
		...
		if (state != State.DISABLED) {
			throw new IllegalStateException("This core.window was already enabled");
		}
		...
		show();
		updateViewportSize();
		Log.info("Enabled and show core.window {} and make it to the current OpenGL context.", windowId);
		return this;
	}

	public Window disable() {
		if (state == State.DISABLED) {
			throw new IllegalStateException("This core.window was already disabled");
		}
	...
	public void hide() {
		if (isVisible) {
			...
			isVisible = false;
			Log.debug("Hide the core.window {}.", windowId);
		}
	}
	...
	public void show() {
		if (!isVisible) {
			...
			isVisible = true;
			Log.debug("Show the core.window {}.", windowId);
		}
	}
